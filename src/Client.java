...
public class Client {
	...
	public static ArrayList<int[][]> autresEquipes = new ArrayList<>(); //liste car on connait pas le nombre d'equipe
	...
	public static ArrayList<int[]> listeObjectifs = new ArrayList<>(); //stock les x et y de tous les objectifs
	...
	public static void main(String[] args) throws Exception{
		...
		outs = new PrintWriter(new BufferedWriter(new OutputStreamWriter(s.getOutputStream())), true);
		...
		while(!data.equals("FIN")){
			...
			for(int index = 0; index < equipe.length; index++){ //pour chaque joueur
				equipe[index].listeOuverte = new HashMap<>();
				equipe[index].listeFermee = new HashMap<>();
				equipe[index].chemins = new ArrayList<>();
			}
			autresEquipes = new ArrayList<>();
	...
	public static void parseReponse(String reponse){
		...
		for(int i = 3; i < nbEquipes + 3; i++){
			...
			if(num == i - 3){ //c'est notre equipe
				...
				for(int j = 2; j < 5; j++){ //la liste des joueurs commence à l'indice 2
					parametres = new int[3];
					joueur = parse[j].split(":");
					parametres[0] = Integer.parseInt(joueur[1]);
					parametres[1] = Integer.parseInt(joueur[2]);
					try{
						parametres[2] = Integer.parseInt(joueur[3]); //si ca ne leve pas d'exception alors il a un fruit
					}catch(NumberFormatException e){
						parametres[2] = -1; //sinon il n'a pas de fruit
					}
					equipeAdverse[j - 2] = parametres;
				}
				autresEquipes.add(equipeAdverse); //on ajoute l'equipe
			}
